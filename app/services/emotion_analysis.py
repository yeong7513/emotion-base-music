...
async def extract_keywords(user_input: str):
    """Asynchronous function to extract keywords from a given text."""
    try:
        # Trim the text to 300 characters, ensuring words are not cut off
        if len(user_input) > 300:
            text = user_input[:300].rsplit(" ", 1)[0]  # Prevents breaking words in the middle
